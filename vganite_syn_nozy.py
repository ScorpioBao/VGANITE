...
def vganite_syn_nozy(train_x,train_t,train_y,train_potential_y,test_x,test_potential_y,y_test,test_t,parameters):
    ...
    class VAE(tf.keras.Model):
        ...
        def encoder(self,x):
            ...
            return mu_t,log_var_t,mu_c,log_var_c

        def decoder(self,z_t,z_c):
            Z = tf.concat([z_t,z_c],axis=1)
            Z = self.fc4(Z)
            x = self.fc4_1(Z)
            x = self.fc41(x)
            return x
        ...
        def call(self,inputs,t,y,training=None):
            mu_t,log_var_t,mu_c,log_var_c, = self.encoder(inputs)
            ...
            x_hat = self.decoder(z_t,z_c)
            t_hat = self.decoder_t(z_t,z_c)
            y_hat = self.generator(z_c)
            treat_t_hat = self.treatment(z_c)

            return z_c,x_hat,t_hat,y_hat,mu_t,log_var_t,mu_c,log_var_c,treat_t_hat
    ...
    class discriminator(tf.keras.Model):
        ...
        def call(self,z_c,t,y,y_hat):
            ...
            inputs = tf.concat([z_c,input0,input1],axis=1)#[X,Y(0),Y(1)]
        ...
        for it in range(iterations):
            for _ in range(2):
                with tf.GradientTape() as tape:
                    ...
                    z_c,x_hat,t_hat,y_hat,mu_t,log_var_t,mu_c,log_var_c,treat_t_hat= model(x,t,y)
                    Y_tilde = tf.nn.sigmoid(y_hat)
                    D_logit = Discriminator(z_c,t,y,Y_tilde)
            ...
            with tf.GradientTape() as tape:
                ...
                loss = 1e-4*rec_x_loss + 1e-4*rec_t_loss + 1e-4*(kl_div_t+kl_div_c) + G_loss
    ...
    t = tf.cast(t, tf.float32)
    y = tf.cast(y, tf.float32)
    z_c,  x_hat, t_hat, y_hat_factual, mu_t, log_var_t, mu_c, log_var_c,   treat_t_hat = model(x,t,y)
    for it in range(iterations):
        with tf.GradientTape() as tape:
