...
def vganite_syn(train_x,train_t,train_y,train_potential_y,test_x,test_potential_y,y_test,test_t,parameters):
    ...
    for epoch in range(1):
        for it in range(iterations):
            ...
            with tf.GradientTape() as tape:
                ...
                loss =  rec_x_loss + rec_t_loss +  treat_t_hat_loss + 1e-4*(kl_div_t+kl_div_c+kl_div_y) + G_loss
            ...
            if it % 100 == 0:
                ...
                # t = tf.cast(t, tf.float32)
                # y = tf.cast(y, tf.float32)
                # z_c, z_y, x_hat, t_hat, y_hat, mu_t, log_var_t, mu_c, log_var_c, mu_y, log_var_y = model(x,t,y)
                # y_hat = tf.cast(y_hat, tf.float32)
