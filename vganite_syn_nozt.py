...
def vganite_syn_nozt(train_x,train_t,train_y,train_potential_y,test_x,test_potential_y,y_test,test_t,parameters):
    ...
    class VAE(tf.keras.Model):
        ...
        def encoder(self,x):
            ...
            return mu_c,log_var_c,mu_y,log_var_y

        def decoder(self,z_c,z_y):
            Z = tf.concat([z_c,z_y],axis=1)
            Z = self.fc4(Z)
            x = self.fc4_1(Z)
            x = self.fc41(x)
            return x

        def decoder_t(self,z_c):
            t_hat = self.fc5(z_c)
        ...
        def call(self,inputs,t,y,training=None):
            mu_c,log_var_c,mu_y,log_var_y = self.encoder(inputs)
            ...
            x_hat = self.decoder(z_c,z_y)
            t_hat = self.decoder_t(z_c)
            y_hat = self.generator(z_c,z_y)
            treat_t_hat = self.treatment(z_c)

            return z_c,z_y,x_hat,t_hat,y_hat,mu_c,log_var_c,mu_y,log_var_y,treat_t_hat
        ...
        for it in range(iterations):
            for _ in range(2):
                with tf.GradientTape() as tape:
                    ...
                    z_c,z_y,x_hat,t_hat,y_hat,mu_c,log_var_c,mu_y,log_var_y,treat_t_hat= model(x,t,y)
            ...
            with tf.GradientTape() as tape:
                ...
                loss = 1e-4*rec_x_loss + 1e-4*rec_t_loss + 1e-4*(kl_div_c+kl_div_y) + G_loss
    ...
    t = tf.cast(t, tf.float32)
    y = tf.cast(y, tf.float32)
    z_c, z_y, x_hat, t_hat, y_hat_factual,   mu_c, log_var_c, mu_y, log_var_y, treat_t_hat = model(x,t,y)
    for it in range(iterations):
        with tf.GradientTape() as tape:
